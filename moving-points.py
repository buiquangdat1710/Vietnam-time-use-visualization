...
def patch_html_file():
    with open('./moving_point.html', 'r') as file:
        content = file.read()
    
    # Thay thế hàm minutesToTime
    new_minutesToTime = """
function minutesToTime(m) {
    var minutes = (m + 4.0*60) % 1440;
    ...
    content = content.replace(
        content[content.find('function minutesToTime'):content.find('// SAVE CHART TO SVG')], 
        new_minutesToTime
    )
    
    with open('./moving_point.html', 'w') as file:
        file.write(content)
...
d3.movingbubbles(data, 
    ...
    figsize=(780, 800), 
    size=2,
    # Thêm tham số để giữ nguyên file HTML
    overwrite=False  # Nếu thư viện hỗ trợ
)
patch_html_file()